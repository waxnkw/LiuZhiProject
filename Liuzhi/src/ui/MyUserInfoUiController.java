...
public class MyUserInfoUiController {
	 @FXML
	 private JFXMasonryPane masonryPane;
	    ...
	    @PostConstruct
	    public void init() {
	    	String[] cmt ={"大好人","热心肠","加油","守信用","超级棒"}; 
	        ArrayList<Node> children = new ArrayList<>();
	        for (int i = 0; i < 5; i++) {
	            StackPane child = new StackPane();
	            double width = Math.random() * 60 + 100;
	            child.setMinWidth(width);
	            child.setMaxWidth(width);
	            child.setPrefWidth(width);
	            double height = Math.random() * 60 + 100;
